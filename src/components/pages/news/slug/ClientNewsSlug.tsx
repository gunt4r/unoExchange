...
import { useArticleBySlug } from '@/queries/useArticles';
...
export default function NewsPageSlug() {
  ...
  const { data: news, isLoading, isError, isPending } = useArticleBySlug(slug as string || '');

  if (!news) {
    return (
      <div className="py-20 text-center">
        <h1 className="text-2xl font-bold">{t('no_data')}</h1>
        <MyLink href="/news" className="mt-4 inline-block" opacity>
          Back to News
        </MyLink>
      </div>
    );
  ...
  if (isError) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="mt-4 text-gray-600">{t('error')}</p>
        <MyLink href="/news" className="mt-4 inline-block" opacity>
          Back to News
        </MyLink>
      </div>
      ...
      <article className="mx-auto max-w-4xl px-4 py-8">
        <nav className="mb-4 text-sm text-gray-600" aria-label="Breadcrumb">
          <ol className="flex items-center gap-2">
            ...
            <li>/</li>
            <li><MyLink href="/news" opacity>News</MyLink></li>
            <li>/</li>
            <li className="text-gray-900" aria-current="page">{news.title}</li>
          </ol>
        </nav>
